...
#        validation_loader_=DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False,  num_workers=NUM_WORKERS,drop_last=True)
        ...
        train_loader_ = DataLoader(dataset, batch_size=args.batch_size, shuffle=False, num_workers=NUM_WORKERS,drop_last=True, sampler=train_sampler_)
        validation_loader_ = DataLoader(dataset, batch_size=args.batch_size, shuffle=False,  num_workers=NUM_WORKERS,drop_last=True,sampler=validation_sampler_)
        ...
        for epoch in range(args.epochs):
            ...
            for batch in train_loader_:
                ...
                for sample in samples:
                    ...
                    # y_ = sample.y.cpu().numpy()
                    #
                    # print(f'sample.y.cpu(): {y_}')
            ...
            for batch in validation_loader_:
                ...
                y_=batch.y.cpu().numpy()
                y_array.append(y_)
                pred__=pred_.cpu().numpy()
                pred_array.append(pred__)
                # prec_, rec_, fscore_, _ = precision_recall_fscore_support(y_, pred__, average=None)
                # print(f'prec_:{prec_},rec_:{rec_},fscore_:{fscore_}')
                correct_ += pred_.eq(batch.y).sum().item()
                print(f'bath.y:{batch.y}, pred_:{pred_}')

            correct_ /= len(validation_fold_idx_)

            prec_, rec_, fscore_, _ = precision_recall_fscore_support(y_array, pred_array, average='weighted')
            prec_array.append(prec_)
            rec_array .append(rec_)
            F1_array .append(fscore_)
            print(f'prec_:{prec_},rec_:{rec_},fscore_:{fscore_}')

            ConfusionMatrix_csv=pd.DataFrame({'y_true':y_array,'y_pred':pred_array})
            ConfusionMatrix_csv['epoch']=epoch
            ConfusionMatrix_csv['fold']=fold

            #summery the model param
            #torchsummary.summary(model_,(48,32),batch_size=1)


            #generate the CFMX
            if fold == 0:
                ConfusionMatrix_csv.to_csv('./output/Confusion_MX.csv',header=True,sep=',', index=None)
            else:
                ConfusionMatrix_csv.to_csv('./output/Confusion_MX.csv', header=False, sep=',', index=None,mode='a+')
        ...
        Eval_graph_csv=pd.DataFrame({'epoch':epoch_cnt, 'accuracy':validation_accuracies_,'precision':prec_array,'recall':rec_array,'F1':F1_array})
        Eval_graph_csv['fold']= fold
        if fold == 0:
            Eval_graph_csv.to_csv('./output/Eval_Graph.csv',header=True,sep=',', index=None)
        else:
            Eval_graph_csv.to_csv('./output/Eval_Graph.csv', header=False, sep=',', index=None,mode='a+')
...
def train(args, experimentStr):
    ...
    biotacsp_dataset_ = dataset.biotacsp.BioTacSp(root='data/biotacsp', k=args.graph_k, split="train",normalize=False) #normalization=True
...
if __name__ == "__main__":
    ...
    parser_.add_argument("--graph_k", nargs="?", type=int, default=2, help="K-Neighbours for graph connections, use 0 for manual connections")
    ...
    parser_.add_argument("--lr", nargs="?", type=float, default=0.0001, help="Learning Rate")#0.0001
    parser_.add_argument("--epochs", nargs="?", type=int, default=24, help="Training Epochs")#16
